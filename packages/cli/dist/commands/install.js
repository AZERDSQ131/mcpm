...
export async function install(serverIds) {
    ...
    if (detectedClients.length === 0) {
        ...
        console.log(chalk.dim("Supported: Claude Code, Cursor, VS Code Copilot, Windsurf, Codex\n"));
        return;
    }
...
async function installOne(serverId, clients) {
    const server = getServer(serverId);
    if (!server) {
        console.log(chalk.red(`✗ Unknown server: ${chalk.bold(serverId)}`), chalk.dim(`— run ${chalk.italic("mcpm search")} to browse available servers`));
        return;
    }
    ...
    if (successCount > 0) {
        ...
        if (envKeys.length > 0) {
            console.log(chalk.dim(`  Restart your AI client for changes to take effect.`));
        }
    }
    else {
